...
import { LineChart2D } from "@/charts/LineChart2D";
import type { ChartHoverPayload, LineChartData } from "@/charts/types";
...
let chart: LineChart2D | null = null;
let ro: ResizeObserver | null = null;

onMounted(() => {
  if (!canvasRef.value) return;
  chart = new LineChart2D(canvasRef.value, {
    minZoomSpan: 3,
    onHover: (payload) => {
      hover.value = payload;
    }
...
.chart-canvas{
  ...
  cursor: crosshair;
  background:
    radial-gradient(circle at 20% 12%, rgba(103,80,164,.08), transparent 45%),
    linear-gradient(180deg, #fefcff, #ffffff);
}
.tooltip{
  position: absolute;
  pointer-events: none;
